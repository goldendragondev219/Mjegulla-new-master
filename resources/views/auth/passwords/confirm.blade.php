      ...
      <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
              <h1 class="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">
                ...
                @if(session('error'))
                    <span class="block text-red-600 text-sm font-medium mt-2">{{ session('error') }}</span>
                @endif
              <form class="space-y-4 md:space-y-6" method="POST" action="{{ route('password_update') }}">
                @csrf
                    <div>
                        <label for="code" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ trans('general.code') }}</label>
                        <input type="text" name="page_key" value="{{ $page_key }}" id="page_key" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" style="display: none;">
                        <input type="code" name="code" id="code" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{ trans('general.code') }}" required="">
                            @error('code')
                  ...
                  <div>
                    ...
                    @error('password')
                        <span class="block text-red-600 text-sm font-medium mt-2">{{ $message }}</span>
                     @enderror
                  <button type="submit" class="res_passw-full text-white bg-blue-500 hover:bg-blue-600 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-700 dark:hover:bg-blue-800 dark:focus:ring-blue-800">{{ trans('general.update') }}</button>

...
.centered-container {
    ...
    height: 30vh; 
                    ...
                    <button class="btn btn-primary mb-3" id="create_ticket" data-toggle="modal" data-target="#ticketModal">Create Ticket</button>
                    <!-- DataTales Example -->
                    <div class="card shadow mb-4">
                        ...
                        @if($tickets->isEmpty())
                            <div class="centered-container">
                                ...
                                <label><h5>There are no tickets!</h5></label>
                            </div>
                        @else
                            @foreach ($tickets as $ticket)
                            <div class="col-12 py-2">
                                <div class="card" onclick="window.location='support/ticket/{{ $ticket->ticket_id }}'" style="cursor: pointer;">
                                    <div class="card-body d-flex justify-content-between align-items-center settings-profile" style="background-color: @if(auth()->user()->dark_mode == '1') #2a2e39; @else #e9ecef; @endif border-radius: 5px;">
                                        <h5 class="card-title mb-0">{{ $ticket->title }} @if($ticket->admin_seen == 0)<span class="badge badge-primary">Unread</span>@endif - {{ $ticket->user->name }} - ( {{ $ticket->user->email }} )</h5>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="submit" class="btn btn-primary" form="ticketForm">Send</button> 
